...
@ConsistentCopyVisibility // this makes the visibility of .copy() also private, same as the constructor
data class FListKind<T : Any> private constructor(
    ...
    /** See [IdMe.show] */
    override val show by lazy {
        (foldLeft("${FList::class.simpleName}@{$size}:") { str, h -> "$str($h, #" }) + "*)".repeat(size)
    }
    ...
    override fun equals(other: Any?): Boolean = this === other || (other is FListKind<*>) && run({
        ...
        // if hash is the same, then continuousMatches must equal size
        // if hash is different, then continuousMatches must differ from size
        assert( continuousMatches(this,other).let { (equality && (size == it)) || (!equality && (size != it)) })
            {(object : FLibErrLog {}).errLog(
                "lh=$hash,\nrh=${other.hash},\nthis =$show,\nother=${other.show}",this@FListKind)
            }
        return equality
    })
    ...
    companion object {
         ...
         * different.
         */
        internal tailrec fun continuousMatches(lhsTail: FListKind<*>, rhsTail: FListKind<*>, matchCount: Int = 0): Int =
            when {
