...
import org.xrpn.flib.internal.impl.FLK
...
fun <TT: Any> FListNonEmpty<TT>.count(isMatch: (TT) -> Boolean): Int = FLKPatterns.k2c<TT>(this.kind as FLK<TT>).let{ it.ops.fcount(it,isMatch)}

fun <T: Any> KFList<T>.equal(rhs: KFList<T>): Boolean = equals(rhs)
fun <TT: Any> FListNonEmpty<TT>.equal(rhs: FListNonEmpty<TT>): Boolean = FLKPatterns.k2c<TT>(this.kind as FLK<TT>).ops.equals(rhs.kind as FLK<TT>)
...
fun <TT: Any, BB> FListNonEmpty<TT>.foldLeft(z: BB, f: (BB, TT) -> BB): BB = FLKPatterns.k2c<TT>(this.kind as FLK<TT>).let { it.ops.ffoldLeft(it,z,f) }
...
fun <TT: Any> FListNonEmpty<TT>.prepend(item: TT): FListNonEmpty<TT> = KFList.of(FLCons(item,(this as FNel).nel)).fnel()
